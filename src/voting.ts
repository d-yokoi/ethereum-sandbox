...
const main = async (): Promise<void> => {
  ...
  const sendOptions = { from: accounts[0], gas: 1500000, gasPrice: web3.utils.toWei('0.00003', 'ether') };
  const instance = await contract.deploy({ data: bytecode, arguments: [candidatesHex] }).send(sendOptions);
